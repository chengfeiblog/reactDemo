      ...
      <div className="container-fluid">
        ...
        <nav>
          <ul className="pager">
            ...
            <li className={'pager-next' + (repos.length > 0 ? '' : ' disabled')}><a href="#" onClick={this.handleNextPageClick}>下一页</a></li>
          </ul>
        </nav>

        {
          error &&
            <div className="alert alert-danger">
              {error.message || '不知道的错误.'}
            </div>
        }

        {!isFetching && repos.length === 0 &&
          <div className="alert alert-warning">数据暂无</div>
        }
        ...
        {repos.length > 0 &&
          <div ref="TABLE_DIV" style={{ opacity: isFetching ? 0.5 : 1 }}>
            <Table
              ...
              <Column
                header={<Cell>拥有者</Cell>}
                cell={<OwnerCell data={repos} col="owner"/>}
                width={200}
              />

              <Column
                header={<Cell>看星星的人</Cell>}
